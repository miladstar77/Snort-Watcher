...
if not path.exists("total_old.txt"):
    snort_db_cursor = snort_db.cursor()
    snort_db_cursor.execute("select count(*) from acid_event ")
    for x in snort_db_cursor:
...
if int(old_total) == new_total:
    ...
    for x in snort_db_cursor:
        # print(x['sig_class_id'],x['sig_name'])
        if x['sig_class_id'] in sig_class:
            # print("sig_class-id : ", x['sig_class_id'])
            x_str = str(x['sig_class_id'])  # change int to str for use in dict and find name of class
            message = "Generated by Snort-Watcher {} \n \n#ID : {}  | Time : {} | Class : {} | Signature_name:*** {} " \
                      "*** | {} -> {} | Total_alert : {}".format(
                current_local_time, x['cid'], x['timestamp'], sig_class_name[x_str], x['sig_name'],
                x['INET_NTOA(ip_src)'], x['INET_NTOA(ip_dst)'], new_total)
